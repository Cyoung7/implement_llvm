...
该表达式定义了一个新变量（在这种情况下为“i”），它从起始值迭代，而条件（在这种情况下为“i <n”）为真，递增一个可选步长值（在这种情况下为“1.0”））。 如果省略步长值，则默认为1.0。 循环为true时，它会执行其正文表达式。 因为我们没有更好的办法，所以我们只需将循环定义为始终返回0.0。 将来当我们有可变变量时，它会变得更有用。
